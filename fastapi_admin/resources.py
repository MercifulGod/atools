...
class Model(Resource):
    ...
    async def get_inputs(cls, request: Request, obj: Optional[TortoiseModel] = None):
        ...
        for field in cls.get_fields(is_display=False):
            ...
            ret.append(await input_.render(request, getattr(obj, name, None), obj))
        return ret
    ...
    def _get_display_input_field(cls, field_name: str) -> Field:
        ...
        label = field_name
        null = field.null or field.default is not None
    ...
    @abc.abstractmethod
    async def post_save(self, request: Request, obj: Optional[TortoiseModel] = None):
        """ model 保存后触发 """
        pass
