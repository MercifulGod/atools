...
async def list_view(
    ...
    # qs = model.all()
    qs = model_resource.get_queryset(request)
...
async def update(
    ...
    async with in_transaction() as conn:
        ...
        await model_resource.post_save(request, obj)
...
async def create(
    ...
    data, m2m_data = await model_resource.resolve_data(request, form)
    async with in_transaction() as conn:
        obj = await model.create(**data, user_id=request.state.admin.id, using_db=conn)
        for k, items in m2m_data.items():
...
async def delete(request: Request, pk: str, model: Model = Depends(get_model)):
    if "is_delete" in model._meta.fields:
        await model.filter(pk=pk).update(is_delete=True)
...
async def bulk_delete(request: Request, ids: str, model: Model = Depends(get_model)):
    if "is_delete" in model._meta.fields:
        await model.filter(pk__in=ids.split(",")).update(is_delete=True)
    return RedirectResponse(url=request.headers.get("referer"), status_code=HTTP_303_SEE_OTHER)
